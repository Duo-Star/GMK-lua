...
GMT.__index.tangent=function(gmt,name,p,geo)
  gmt.step[#gmt.step+1]=name
  gmt:add(name,{
    name=name,
    class="Line",
...
GMT.__index.angleBisector=function(gmt,name,F1,F2,F3)
  gmt.step[#gmt.step+1]=name
  gmt:add(name,{
    name=name,
    class="Line",
    method="angleBisector",
    factor={F1,F2,F3},--这里存放的是字符串
    free=true,
    g={color="teal",width=8.0,style="STROKE",label=true,fill=false,show=true}
  })
end
...
GMT.__index.addSlider=function(gmt,name,info,actting)
  gmt.step[#gmt.step+1]=name
  gmt:add(name,{
    name=name,
    class="Slider",
  ...
  if class=="Point"
   ...
   elseif class=="Line"
    if method=="new"
     ...
     elseif method=="angleBisector"
      result="以 "..factor[2].." 为顶点，两边过 "..factor[1]..", "..factor[3].."的角的平分线"
     elseif method=="tangent"
      result=""..factor[2].." 在 "..factor[1].."处的切线"
    end
   elseif class=="Circle"
    if method=="new"
   ...
   elseif class=="Slider"
    if method=="distribute"
      result="在 ".."[ "..factor.info.min..","..factor.info.max.." ]".."区间，以 "..factor.info.type.." 运动，速度为 "..factor.info.speed.." 的变值"
      .."\n".."作用在: "
      for i=1,#factor.actting-1 do
        local item=factor.actting[i]
        result=result..item.name..", "
      end
    result=result..factor.actting[#factor.actting].name.." 上"
    end
...
GMT.__index.run=function(gmt,data)
  ...
  for i=1,#gmt.step do
    ...
    local item=gmt[a]
    if item
      local d_=gLib[item.class][item.method](item.factor,data,gmt)
      if d_
        data[a]=d_
       else
        gmt:delete(data,a)--依赖对象未定义, 删除此对象
      end
    end
  end
