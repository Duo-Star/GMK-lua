...
graph.makePaint_2=function(g)
  return Paint().setColor(graph.color[g.color][2]).setStyle(Paint.Style[g.style]).setStrokeWidth(g.width).setAntiAlias(true).setStrokeCap(Paint.Cap.ROUND)
end


graph.drawPoint=function(canvas,v,p_)
  local p_=p_ or paint
  canvas.drawCircle(v.x*graph.lam+graph.o.x, v.y*(-graph.lam)+graph.o.y, 12, p_)
end
