...
TC.disCircle=function(graph,gtp,item)
  return (math.abs(#(gtp-item.p)-item.r))*graph.lam
end

TC.disPoint=function(graph,gtp,item)
  return (math.abs(#(gtp-item.p)-item.r))*graph.lam
end


TC.choose=function(gtp,dx,info,todo)
  for i=#gmt.step,1,-1 do
    local a=gmt.step[i]
    local item=data[a]
    local class=getmetatable(item)--获得到对象的类
    ...
    if tool==toolList.Move--选择移动工具
     ...
     elseif tool==toolList.Line--选择直线工具💰💰💰💰💰💰💰
      TC.tcP(gtp,75,function(a,dx)--构造直线
        if toolInfo.L.DOWN.P1~="" and a~=toolInfo.L.DOWN.P1
     ...
     elseif tool==toolList.Circle
      TC.tcP(gtp,75,function(a,dx)
        if toolInfo.C.DOWN.P1~="" and a~=toolInfo.C.DOWN.P1
     ...
     elseif tool=="MidP"
      TC.tcP(gtp,75,function(a,dx)
        if toolInfo.MidP.P1~="" and a~=toolInfo.MidP.P1
          local name=newName_P()
          toolInfo.MidP.P2=a
     ...
     elseif tool=="Hyperbola"
      TC.tcP(gtp,75,function(a,dx)
        if toolInfo.Hyperbola.P1~="" and toolInfo.Hyperbola.P2~=""
          local name=newName_C()
          toolInfo.Hyperbola.P3=a
          gmt:addHyperbola(name,toolInfo.Hyperbola.P1,toolInfo.Hyperbola.P2,toolInfo.Hyperbola.P3)
          toolInfo.Hyperbola.P1=""
          toolInfo.Hyperbola.P2=""
          toolInfo.Hyperbola.P3=""
          cleanSelect() guide_card_set(1,"双曲线"..name.."已创建完毕") guide_card_dismiss(1000)
          tool=toolList.Move
          toolTab.selectTab(toolTabList[1])
         elseif toolInfo.Hyperbola.P1~="" and toolInfo.Hyperbola.P2==""
          toolInfo.Hyperbola.P2=a
          selectGeo(a) guide_card_set(0.66,"选择双曲线上一点(2/3)") guide_card_appear()
         elseif toolInfo.Hyperbola.P1=="" and toolInfo.Hyperbola.P2==""
          toolInfo.Hyperbola.P1=a
          selectGeo(a) guide_card_set(0.33,"选择另外的焦点(1/3)") guide_card_appear()
        end
      end)
     ...
     elseif tool=="AngleBisector"
      TC.tcP(gtp,75,function(a,dx)
        if toolInfo.AngleBisector.P1~="" and toolInfo.AngleBisector.P2~=""
          local name=newName_L()
          toolInfo.AngleBisector.P3=a
          gmt:angleBisector(name,toolInfo.AngleBisector.P1,toolInfo.AngleBisector.P2,toolInfo.AngleBisector.P3)
          toolInfo.AngleBisector.P1=""
          toolInfo.AngleBisector.P2=""
          toolInfo.AngleBisector.P3=""
          cleanSelect() guide_card_set(1,"角平分线"..name.."已创建完毕") guide_card_dismiss(1000)
          tool=toolList.Move
          toolTab.selectTab(toolTabList[1])
         elseif toolInfo.AngleBisector.P1~="" and toolInfo.AngleBisector.P2==""
          toolInfo.AngleBisector.P2=a
          selectGeo(a) guide_card_set(0.66,"选择另外一点(2/3)") guide_card_appear()
         elseif toolInfo.AngleBisector.P1=="" and toolInfo.AngleBisector.P2==""
          toolInfo.AngleBisector.P1=a
          selectGeo(a) guide_card_set(0.33,"选择顶点(1/3)") guide_card_appear()
        end
      end)
     elseif tool=="Tangent"
      if toolInfo.Tangent.P~=""
        for i=#gmt.step,1,-1 do
          local a=gmt.step[i]
          local item=data[a]
          local class=getmetatable(item)--获得到对象的类
          if class==Circle and (math.abs(#(gtp-item.p)-item.r))*graph.lam+10<70
            toolInfo.Tangent.geo=a
            --print_(toolInfo.Tangent.P..", "..toolInfo.Tangent.geo)
            local name=newName_L()
            gmt:tangent(name,toolInfo.Tangent.P,toolInfo.Tangent.geo)

          end
        end
       else
        TC.tcP(gtp,75,function(a,dx)
          toolInfo.Tangent.P=a
          selectGeo(a) guide_card_set(0.5,"选择一个图形") guide_card_appear()
        end)
      end
